...
namespace DigitalArena
{
    ...
    {
        [Serializable] sealed class Settings { public string baseUrl = ""; }
        [Serializable] sealed class BeginRequest { public string provider; }
        [Serializable] sealed class BeginResponse { public string attemptId, pollToken, authorizeUrl; }
        [Serializable] sealed class PollRequest { public string attemptId; }
        [Serializable] sealed class PollResponse { public string status, token; public ArenaAccountProfile profile; }
        [Serializable] sealed class NicknameRequest { public string nickname; }
        [Serializable] sealed class Availability { public string nickname, message; public bool available; }
        [Serializable] sealed class ErrorResponse { public string message; }
        ...
        IEnumerator LoginFlow(string provider)
        {
            ...
            yield return Request("/auth/begin", "POST", JsonUtility.ToJson(new BeginRequest { provider = provider }), null,
                value => start = JsonUtility.FromJson<BeginResponse>(value));
            if (start == null) { Busy = false; yield break; }
            ...
            { Message = "로그인 주소를 검증하지 못했습니다."; Busy = false; yield break; }
            ...
            while (Time.realtimeSinceStartupAsDouble < deadline)
            {
                ...
                yield return Request("/auth/poll", "POST", JsonUtility.ToJson(new PollRequest { attemptId = start.attemptId }), start.pollToken,
                    value => result = JsonUtility.FromJson<PollResponse>(value));
                ...
                { Message = "계정 정보를 확인하지 못했습니다."; Busy = false; yield break; }
                token = result.token; Profile = result.profile;
        ...
        {
            CheckedNickname = null; NicknameAvailable = false;
        ...
        public void CheckNickname(string value)
        {
            ...
            Busy = true; CheckedNickname = null; NicknameAvailable = false;
        ...
        IEnumerator CheckFlow(string value)
        {
            yield return Request("/nicknames/availability?nickname=" + UnityWebRequest.EscapeURL(value), "GET", null, token, json =>
            {
                ...
                CheckedNickname = value; NicknameAvailable = result.available; Message = result.message;
            });
            Busy = false;
        }
        public void SetNickname(string value)
        {
            if (Busy || !NicknameAvailable || CheckedNickname != value || Profile == null) return;
            Busy = true; StartCoroutine(SetNicknameFlow(value));
        }
        IEnumerator SetNicknameFlow(string value)
        {
            yield return Request("/me/nickname", "POST", JsonUtility.ToJson(new NicknameRequest { nickname = value }), token,
                json => { Profile = JsonUtility.FromJson<ArenaAccountProfile>(json); Message = "가입이 완료되었습니다."; });
            NicknameAvailable = false; CheckedNickname = null; Busy = false;
        }
        public void RefreshProfile()
        {
            if (Busy || Profile == null) return;
            Busy = true; StartCoroutine(RefreshFlow());
        }
        ...
        {
            if (Busy) return;
            Busy = true; StartCoroutine(LogoutFlow());
        }
        IEnumerator LogoutFlow()
        {
            yield return Request("/auth/logout", "POST", "{}", token, _ => { });
            Profile = null; token = null; CheckedNickname = null; NicknameAvailable = false;
            Busy = false; Message = "계정을 선택해 로그인하거나 가입하세요.";
        }
        public void CancelLogin()
        {
            if (Profile != null) return;
            activeRequest?.Abort(); StopAllCoroutines(); activeRequest = null;
            Busy = false; Message = "로그인을 취소했습니다. 계정을 다시 선택할 수 있습니다.";
        }
        void OnDestroy() { activeRequest?.Abort(); }
        ...
        IEnumerator Request(string path, string method, string body, string bearer, Action<string> success)
        {
            using (var request = new UnityWebRequest(baseUrl + path, method))
            {
                ...
                try { success(request.downloadHandler.text); }
                catch (Exception) { Message = "서버 응답을 확인하지 못했습니다. 다시 시도해 주세요."; }
            }
        }
    }
}
