...
namespace DigitalArena
{
    public sealed partial class DigitalArenaGame
    {
        ...
        {
            if (mainMenu || help || rules == null || rules.Health <= 0) return;
            if (Keyboard.current?.escapeKey.wasPressedThisFrame == true)
            {
                CancelDrag(); shopOpen = false; return;
            }
            var mouse = Mouse.current;
            if (mouse == null || SuppressTouchMouse) return;
            var point = mouse.position.ReadValue();
            point = UiPoint(new Vector2(point.x, Screen.height - point.y));
            if (mouse.rightButton.wasPressedThisFrame) HandlePointer(EventType.MouseDown, 1, 1, point);
            ...
            if (mouse.leftButton.wasReleasedThisFrame) HandlePointer(EventType.MouseUp, 0, 1, point);
        }
    }
}
