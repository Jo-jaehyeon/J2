...
BASE = Path(r'C:/Jerry/Unity Project/J2/Assets/Resources/Digimon/5코스트/WarGreymon')
...
for s in [-1,1]:
    plate('EyeSocket',[(s*.065,2.512),(s*.185,2.57),(s*.169,2.494),(s*.089,2.474)],-.204,.018,dark,'Head',.002)
    plate('EmeraldEye',[(s*.096,2.515),(s*.163,2.544),(s*.15,2.501),(s*.105,2.49)],-.226,.008,green,'Head',.001)
    ellipsoid('EyePupil',(s*.133,-.239,2.517),(.011,.004,.018),dark,'Head',8,5)
    plate('EyeGlint',[(s*.111,2.516),(s*.12,2.519),(s*.12,2.499),(s*.111,2.499)],-.236,.003,white,'Head',0)
    mesh('SideEyeRecess',[(s*.194,-.105,2.535),(s*.211,-.022,2.56),(s*.209,.005,2.519),(s*.192,-.081,2.491)],[(0,1,2,3) if s>0 else (3,2,1,0)],dark,'Head')
