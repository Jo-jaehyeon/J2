...
namespace J2.MultiplayerMap
{
    ...
    public sealed class MultiplayerMapTrain : MonoBehaviour
    {
        ...
        float[] lengths;
        float totalLength;
        public float RouteLength { get { EnsureRoute(); return totalLength; } }

        void Awake() { EnsureRoute(); ApplyPose(); }
        void Update()
        {
        ...
        {
            if (float.IsNaN(meters) || float.IsInfinity(meters)) return;
            PreviewMovement = false;
            EnsureRoute();
            distance = totalLength > 0 ? Mathf.Repeat(meters, totalLength) : 0;
            ApplyPose();
        }
        public void RebuildRoute() { lengths = null; EnsureRoute(); ApplyPose(); }
        void EnsureRoute()
        {
            if (Route == null || Route.Length < 2) { lengths = null; totalLength = 0; return; }
            if (lengths != null && lengths.Length == Route.Length) return;
            lengths = new float[Route.Length]; totalLength = 0;
            for (int i = 0; i < Route.Length; i++)
            { lengths[i] = Vector3.Distance(Route[i], Route[(i + 1) % Route.Length]); totalLength += lengths[i]; }
        }
        void ApplyPose()
        {
            if (Train == null || lengths == null || totalLength <= 0) return;
            float remaining = Mathf.Repeat(distance, totalLength);
            for (int i = 0; i < lengths.Length; i++)
            {
                if (lengths[i] <= .00001f) continue;
