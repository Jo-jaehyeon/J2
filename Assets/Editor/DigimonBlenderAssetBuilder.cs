...
public static class DigimonBlenderAssetBuilder
{
    ...
    public class Manifest
    {
        ...
        public string	name, korean, costFolder;
        public float	targetHeight, maxWidth, maxDepth;
        public int		triangleBudget;
    }

    public static string[] Manifests() => Directory.GetFiles("Assets/Resources/Digimon", "manifest.json", SearchOption.AllDirectories).Where(p => new DirectoryInfo(Path.GetDirectoryName(p)).Name == "Source~").OrderBy(p => p).ToArray();
