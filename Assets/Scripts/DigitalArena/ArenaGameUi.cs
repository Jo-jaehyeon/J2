...
namespace DigitalArena
{
    ...
    public sealed class ArenaGameUi : IDisposable
    {
        ...
        public void Begin(float scale, Vector2 offset)
        {
            ...
            Enabled = true;
            foreach (var control in buttons.Values) control.Used = false;
        }

        T Take<T>(List<T> pool, ref int count) where T : VisualElement, new()
        {
            if (count == pool.Count)
            {
        ...
        {
            if (element.parent != screen) screen.Insert(order, element);
            else if (screen.IndexOf(element) != order) { element.RemoveFromHierarchy(); screen.Insert(order, element); }
            order++;
            element.style.display = DisplayStyle.Flex;
            element.style.left = rect.x; element.style.top = rect.y;
            element.style.width = Mathf.Max(0, rect.width); element.style.height = Mathf.Max(0, rect.height);
        }
        ...
        public void Picture(Rect rect, Texture texture, ScaleMode mode = ScaleMode.StretchToFill, float opacity = 1)
        {
            ...
            element.image = texture; element.scaleMode = mode; element.style.opacity = opacity;
            Place(element, rect);
        }
        ...
        public void Button(string id, Rect rect, string value, Color color, Action action, bool enabled = true, bool transparent = false, string tooltip = null)
        {
            ...
            {
                control = new Control { Element = new Button { name = id } };
                ...
                {
                    if (captured.Used && captured.Element.enabledInHierarchy) captured.Action?.Invoke();
                };
                buttons.Add(id, control);
            }
            control.Used = true; control.Action = action;
        ...
        public void End()
        {
            HideUnused(boxes, boxCount); HideUnused(labels, labelCount); HideUnused(images, imageCount);
            foreach (var control in buttons.Values)
                if (!control.Used) { control.Element.style.display = DisplayStyle.None; control.Action = null; }
        }

        static void HideUnused<T>(List<T> pool, int count) where T : VisualElement
        {
            for (int i = count; i < pool.Count; i++) pool[i].style.display = DisplayStyle.None;
        }
