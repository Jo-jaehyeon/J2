...
class Detailed(Builder):
    ...
    def apply_surface_colors(self):
        mesh=self.body.data
        unpainted=mesh.color_attributes.get('ReferenceColor') is None
        ...
        for p in mesh.polygons:
            for i in p.loop_indices:
                if unpainted or attr.data[i].color[3]<.5:attr.data[i].color=mesh.materials[p.material_index].diffuse_color
...
def fairy(b):
    ...
    for p in torso.data.polygons:
        ...
        if (.90 if rose else 1.04)<z<1.58:p.material_index=1
    for ob in b.parts:
        if ob.name.startswith('Contoured leg'):
            ob.data.materials.append(b.mat('Peach' if rose else 'Leaf'))
            for poly in ob.data.polygons:
                z=sum(ob.data.vertices[i].co.z for i in poly.vertices)/len(poly.vertices)
                if (z>.80 if rose else z<.60):poly.material_index=1
    if rose:
