...
namespace DigitalArena
{
    ...
    public sealed class ArenaRules
    {
        ...
        public static readonly string[]	CostNames =
        {
            "1코스트",
            "2코스트",
            "3코스트",
        ...
        {
            string catalogError = (catalog ?? new DigimonCatalog()).Validate();

            if (catalogError != null)
            {
                throw new ArgumentException(catalogError);
            }

            Catalog = (catalog ?? new DigimonCatalog()).Copy();
            random = new Random(seed);
            Balance = (balance ?? new ArenaBalance()).Copy();

            string error = Balance.Validate();
        ...
        void RefreshOffers()
        {
            ...
            {
                if (Offers[i] >= 0 && !Bought[i])
                {
                    RemainingPool[Catalog.allies[Offers[i]].cost - 1]++;
                }
            }
        ...
        int DrawTier()
        {
            ...
            for (int c = 0; c < 5; c++)
            {
                if (RemainingPool[c] <= 0 || candidates[c].Length == 0)
                {
                    continue;
                }
                ...
                if (roll < 0)
                {
                    ...
                    return candidates[c][candidates[c].Length == 1 ? 0 : random.Next(candidates[c].Length)];
                }
            }
        ...
        {
            if (!Preparing || index < 0 || index >= 5 || Bought[index])
            {
                return false;
            }
            ...
            if (Gold < PurchasePrice(Catalog.allies[tier]))
            {
                Notice = "골드가 부족합니다.";
            ...
            {
                Notice = "대기석이 가득 찼습니다. 기물을 전장에 배치하세요.";
        ...
        {
            if (!Preparing || Gold < 4 || Level >= MaxLevel || amount <= 0)
            {
                return false;
            }
        ...
        void GrantXp(int amount)
        {
            ...
            while (Level < MaxLevel && total >= RequiredXp)
            {
                total -= RequiredXp;
                Level++;
            }
        ...
        {
            if (!Preparing || cell < -1 || cell >= BoardSize)
            {
                return false;
            }
            ...
            if (cell == -1)
            {
                return MoveToBench(id, FirstBenchSlot());
            }
            ...
            {
                Notice = "배치 한도입니다. XP를 구매해 레벨을 올리세요.";
            ...
            if (occupant != null)
            {
                occupant.Cell = piece.Cell;
                occupant.BenchSlot = piece.BenchSlot;
            }
        ...
        int FirstBenchSlot()
        {
            ...
            {
                if (Pieces.All(p => p.Cell >= 0 || p.BenchSlot != i))
                {
                    return i;
                }
            }
        ...
        {
            if (!Preparing || slot < 0 || slot >= BenchSize)
            {
                return false;
            }
        ...
        {
            if (Round <= 0 || Preparing || battleResolved || Health <= 0)
            {
                return;
            }
            ...
            if (won)
            {
                Notice += " · 승리 +1 골드" + (LastStreakGold > 0 ? " · " + WinStreak + "연승 +1 골드" : "");
            }
        }
    }
}
