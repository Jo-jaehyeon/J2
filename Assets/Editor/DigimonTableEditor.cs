...
public sealed class DigimonTableEditor : EditorWindow
{
    ...
    public static void Open() { GetWindow<DigimonTableEditor>("디지몬 데이터 테이블").minSize=new Vector2(760,670); }
    void OnEnable() { Reload(); }
    void Reload() { data=File.Exists(Path)?JsonUtility.FromJson<DigimonCatalog>(File.ReadAllText(Path)):new DigimonCatalog();selected=0; }
    DigimonData[] Rows => side==0?data.allies:data.enemies;
    void SetRows(DigimonData[] rows) { if(side==0)data.allies=rows;else data.enemies=rows; }
    void Add(bool duplicate)
    {
        var row=duplicate&&Rows.Length>0?Rows[selected].Copy():new DigimonData();
        row.id="digimon_"+Guid.NewGuid().ToString("N");row.name=duplicate?row.name+" 복사":"새 디지몬";
        if(!duplicate) { row.cost=side==0?1:0;row.startStage=1; }
        SetRows(Rows.Concat(new[]{row}).ToArray());selected=Rows.Length-1;
    }
    void OnGUI()
    {
        ...
        if(GUILayout.Button("선택 기물 복제")) Add(true);
        ...
        if(chosen!=prefab)
        {
            ...
            if(chosen==null) row.prefabPath="";
            else if(marker<0||!path.EndsWith(".prefab",StringComparison.OrdinalIgnoreCase)) status="프리팹은 Assets 아래 Resources 폴더 안에 저장하세요.";
            else row.prefabPath=path.Substring(marker+11,path.Length-marker-11-7);
        }
        EditorGUILayout.LabelField("프리팹 경로",string.IsNullOrEmpty(row.prefabPath)?"없음 · 기본 도형 사용":row.prefabPath);
        if(!string.IsNullOrEmpty(row.prefabPath)&&prefab==null&&GUILayout.Button("누락된 프리팹 연결 해제")) row.prefabPath="";
        ...
        using(new EditorGUI.DisabledScope(Rows.Length<=1||referenced))
            if(GUILayout.Button("선택 기물 삭제")) { SetRows(Rows.Where(d=>d!=row).ToArray());selected=0; }
        if(referenced) EditorGUILayout.HelpBox("진화 대상으로 사용 중입니다. 해당 연결을 먼저 변경하면 삭제할 수 있습니다.",MessageType.Info);
        EditorGUILayout.EndScrollView();
        string error=data.Validate();
        if(error==null&&data.allies.Concat(data.enemies).Any(d=>!string.IsNullOrEmpty(d.prefabPath)&&Resources.Load<GameObject>(d.prefabPath)==null)) error="누락된 프리팹 연결을 수정하세요.";
        ...
        using(new EditorGUI.DisabledScope(error!=null))
            if(GUILayout.Button("저장",GUILayout.Height(32))) { File.WriteAllText(Path,JsonUtility.ToJson(data,true));AssetDatabase.ImportAsset(Path);status="저장 완료. 게임 재시작으로 적용하세요."; }
        if(GUILayout.Button("다시 불러오기",GUILayout.Height(32))) Reload();
        if(GUILayout.Button("확률 / 재고 테이블",GUILayout.Height(32))) EditorApplication.ExecuteMenuItem("Digital Arena/Balance Table");
        EditorGUILayout.EndHorizontal();
    }
}
