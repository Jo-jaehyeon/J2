...
public static class WarGreymonAssetBuilder
{
    ...
    static readonly string[] Clips = { "Idle", "Walk", "Attack", "Death" };
    static readonly int[] Starts = { 1, 60, 90, 120 }, Ends = { 49, 84, 108, 150 };

    [MenuItem("Digital Arena/Import WarGreymon From Blender")]
    public static void Build()
    {
        if(EditorApplication.isPlaying) throw new InvalidOperationException("Exit Play mode before importing WarGreymon.");
        ...
        importer.clipAnimations = Clips.Select((name,i) => new ModelImporterClipAnimation {
            name = name, takeName = take, firstFrame = Starts[i], lastFrame = Ends[i],
            loopTime = i < 2, loopPose = i < 2,
            wrapMode = i < 2 ? WrapMode.Loop : WrapMode.ClampForever,
        ...
        if(shader == null) throw new InvalidOperationException("J2 Solid shader is missing.");
        ...
        try
        {
            ...
            foreach(var state in animation.Cast<AnimationState>().ToArray()) animation.RemoveClip(state.name);
            foreach(string name in Clips)
            {
                ...
                clip.name = name; clip.legacy = true;
        ...
        if(catalog.Validate() is string error) throw new InvalidOperationException(error);
    ...
    public static void Validate()
    {
        ...
        if(prefab == null) throw new Exception("WarGreymon prefab missing.");
        var root = UnityEngine.Object.Instantiate(prefab);
        try
        {
            var anim = root.GetComponent<Animation>();
            if(root.GetComponent<ArenaUnitAnimation>() == null) throw new Exception("Arena animation driver missing.");
            foreach(var r in root.GetComponentsInChildren<Renderer>())
                if(r.sharedMaterials.Any(m => m == null || m.shader.name != "DigitalArena/Solid")) throw new Exception("Invalid material.");
            var bones = root.GetComponentsInChildren<Transform>();
            foreach(string name in Clips)
            {
                var clip = anim.GetClip(name);
                if(clip == null || !clip.legacy || clip.length <= 0) throw new Exception("Invalid clip: " + name);
                ...
                if(!changed) throw new Exception("Animation has no motion: " + name);
            ...
            if(mesh.triangles.Length/3 > 15000) throw new Exception("Low-poly triangle budget exceeded.");
            ...
            if(Mathf.Abs(height-2.15f)>.02f) throw new Exception("Incorrect model height: " + height);
