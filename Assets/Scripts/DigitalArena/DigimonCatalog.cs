...
namespace DigitalArena
{
    ...
    public sealed class DigimonCatalog
    {
        ...
        static DigimonData[] Defaults(bool enemy)
        {
            ...
            return rows.Concat(Enumerable.Range(0, 3).SelectMany(line => Enumerable.Range(0, 5).Select(stage =>
            {
                ...
                row.id = prefixes[line] + (stage + 1);
                row.name = names[line][stage];
        ...
        public DigimonCatalog Copy() => new DigimonCatalog
        {
            allies = allies.Select(d => d.Copy()).ToArray(),
            enemies = enemies.Select(d => d.Copy()).ToArray()
        };

        public string Validate()
        {
            if (allies == null || enemies == null || allies.Length == 0 || enemies.Length == 0)
            {
                return "아군과 크립은 각각 최소 1종이 필요합니다.";
            }

            if (allies.Concat(enemies).Any(d => d == null))
            {
                return "비어 있는 기물 데이터입니다.";
            }

            if (allies.Concat(enemies).Select(d => d.id).Distinct().Count() != allies.Length + enemies.Length)
            {
                return "기물 ID는 중복될 수 없습니다.";
            }

            if (!enemies.Any(d => d.startStage == 1))
            {
                return "스테이지 1에서 등장하는 크립이 필요합니다.";
            }

            if (allies.Any(d => d.cost < 1 || d.cost > 5))
            {
                return "플레이어 기물은 1~5코스트여야 합니다.";
            }
            ...
            foreach (var d in allies.Concat(enemies))
            {
                ...
                {
                    return "ID와 이름을 입력하세요.";
                }

                if (d.cost < 0 || d.cost > 5 || d.modelTier < 0 || d.modelTier > 6 || d.startStage < 1)
                {
                    return "코스트 0~5, 기본 외형 0~6, 등장 스테이지 1 이상이어야 합니다.";
                }

                if (d.range < 1 || d.range > 4)
                {
                    return "Attack range must be an integer from 1 to 4.";
                }
                ...
                if (!Enum.IsDefined(typeof(AttackKind), d.attack) || !Enum.IsDefined(typeof(DigimonType), d.type) || !Enum.IsDefined(typeof(DigimonElement), d.element))
                {
                    return "유효하지 않은 분류입니다.";
                }

                if (!Enum.IsDefined(typeof(PlaceholderModel), d.placeholder))
                {
                    return "유효하지 않은 임시 도형입니다.";
                }

                if (!Enum.IsDefined(typeof(DigimonRole), d.role))
                {
                    return "유효하지 않은 역할군입니다.";
                }
            }
            ...
            foreach (var origin in allies)
            {
                ...
                while (!string.IsNullOrEmpty(current.evolvesTo))
                {
                    ...
                    {
                        return "진화 대상이 순환합니다: " + origin.name;
                    }
                    ...
                    if (current == null)
                    {
                        return "존재하지 않는 진화 대상: " + origin.name;
                    }
                }
            }
    ...
    public static class DigimonDamage
    {
        ...
        {
            if (a == DigimonType.Free || b == DigimonType.Free)
            {
                return 1;
            }

            if ((a == DigimonType.Vaccine && b == DigimonType.Virus) || (a == DigimonType.Virus && b == DigimonType.Data) || (a == DigimonType.Data && b == DigimonType.Vaccine))
            {
                return 2;
            }

            if ((a == DigimonType.Unknown && (int)b < 3) || ((int)a < 3 && b == DigimonType.NoData))
            {
                return 1.1f;
            }
        ...
        public static float Calculate(DigimonData source, DigimonData target, float power = 1) => (source.attack == AttackKind.Physical ? source.ATK : source.INT) * power * (100f / (100f + target.DEF)) * TypeMultiplier(source.type, target.type) * ElementMultiplier(source.element, target.element);
    }
}
