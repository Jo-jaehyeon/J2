...
namespace DigitalArena
{
    public sealed class ArenaWorld3D : MonoBehaviour
    {
        ...
        public void Initialize()
        {
            ...
            Camera.backgroundColor = C(0x9BD9E4);
            Camera.cullingMask = 1 << WorldLayer;
            actors = new GameObject("Pieces").transform;
            ...
            highlight.name = "Drop cell highlight";
        ...
        void BuildStage()
        {
            ...
            for (int i = -16; i <= 16; i++)
            {
                Shape(transform, PrimitiveType.Cube, new Vector3(i * 0.7f, 0.16f, 0), new Vector3(0.18f, 0.12f, 1.32f), C(0x695E4B));
            }

            for (int side = -1; side <= 1; side += 2)
            {
                Shape(transform, PrimitiveType.Cube, new Vector3(0, 0.25f, side * 0.48f), new Vector3(25, 0.13f, 0.08f), C(0xBAC9CA));
            }

            for (int i = 0; i < 10; i++)
            {
                Vector3 p = BenchPosition(i);
            ...
            for (int side = -1; side <= 1; side += 2)
            {
                for (int i = 0; i < 5; i++)
                {
                    ...
                    if (i != 2)
                    {
                        Tree(p + new Vector3(side * 1.2f, 0, 0), 2.4f + i * 0.2f);
                    }
                }
            ...
            tram = new GameObject("Moving green lake tram").transform;
            tram.SetParent(transform);
            BuildTram();
        }
        ...
        {
            var root = new GameObject("Submerged leaning power tower").transform;
            ...
            for (int side = -1; side <= 1; side += 2)
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    Shape(root, PrimitiveType.Cube, new Vector3(0, 0.65f + i * height / 4, 0), new Vector3(0.07f, height / 3, 0.07f), C(0x71888B)).localRotation = Quaternion.Euler(0, 0, side * 37);
                }
            }
        ...
        void BuildTram()
        {
            ...
            for (int side = -1; side <= 1; side += 2)
            {
                for (int i = 0; i < 7; i++)
                {
                    Shape(tram, PrimitiveType.Cube, new Vector3(-1.95f + i * 0.65f, 1.2f, side * 0.67f), new Vector3(0.48f, 0.55f, 0.025f), glass);
                }

                for (int wheel = -1; wheel <= 1; wheel += 2)
                {
                    Shape(tram, PrimitiveType.Cylinder, new Vector3(wheel * 1.6f, 0.18f, side * 0.51f), new Vector3(0.48f, 0.13f, 0.48f), C(0x354148)).localRotation = Quaternion.Euler(90, 0, 0);
                }
        ...
        {
            var result = new Texture2D[(enemy ? catalog.enemies : catalog.allies).Length];

            var go = new GameObject("Portrait camera");
            ...
            camera.cullingMask = 1 << WorldLayer;
            camera.clearFlags = CameraClearFlags.SolidColor;
            camera.backgroundColor = C(0x243C3F);
            camera.orthographic = true;
            camera.orthographicSize = 1.25f;
            camera.nearClipPlane = 0.1f;
            camera.farClipPlane = 20;
            camera.transform.position = new Vector3(1000, 2.3f, 4);
            camera.transform.LookAt(new Vector3(1000, 0.95f, 0));
            ...
            {
                var root = new GameObject("Portrait model").transform;

                root.position = new Vector3(1000, 0, 0);
                BuildUnitModel(root, catalog.Get(tier, enemy), enemy);
                ...
                if (renderers.Length > 0)
                {
                    ...
                    camera.transform.position = bounds.center + new Vector3(0, .5f, 4);
        ...
        {
            if (state != rules || trainRound != rules.Round)
            {
                trainRound = rules.Round;
                trainProgress = 0;
            }
            ...
            else
            {
                ...
                foreach (var piece in rules.Pieces.Where(p => p.Cell < 0))
                {
                    Add(piece.Id, piece.Tier, false, BenchPosition(piece.BenchSlot), null);
                }
            }
        }
        ...
        public void Animate(float time, int draggedId, Vector3? dragPoint)
        {
            ...
            foreach (var unit in Units)
            {
                ...
                if (unit.Motion != null)
                {
                    unit.Motion.Pose(moving, unit.Fighter, Time.deltaTime);
                }
                ...
                if (unit.Fighter != null)
                {
                    ...
                    {
                        unit.Root.rotation = Quaternion.LookRotation(travel);
                    }
                    else if (f.Target != null)
                    {
                        ...
                        {
                            unit.Root.rotation = Quaternion.LookRotation(direction);
                        }
                    }
                }
                ...
                if (unit.Motion == null)
                {
                    pos.y += Mathf.Sin(time * 3 + unit.Home.x) * 0.04f;
                }

                if (unit.Motion == null && unit.Fighter != null && unit.Fighter.Flash > 0)
                {
                    pos += unit.Root.forward * (unit.Fighter.Flash * 0.9f);
                }

                if (unit.Id == draggedId && draggedId >= 0 && dragPoint.HasValue)
                {
                    pos = dragPoint.Value + Vector3.up * 0.65f;
                }
        ...
        public int Pick(Vector2 screenTopLeft)
        {
            var u = PickUnit(screenTopLeft);

            return u == null || u.Enemy ? -1 : u.Id;
        }
        ...
        public UnitView PickUnit(Vector2 screenTopLeft)
        {
            ...
            foreach (var unit in Units.Where(u => u.Root.gameObject.activeSelf).OrderBy(u => Vector3.Distance(Camera.transform.position, u.Root.position)))
            {
                ...
                if (new Rect(feet.x - width / 2, head.y - 8, width, feet.y - head.y + 16).Contains(screenTopLeft))
                {
                    return unit;
                }
            }
        ...
        void OnDestroy()
        {
            ...
            if (quitting && !Application.isEditor)
            {
                return;
            }

            foreach (var cam in previousCameras)
            {
                if (cam != null)
                {
                    cam.enabled = true;
