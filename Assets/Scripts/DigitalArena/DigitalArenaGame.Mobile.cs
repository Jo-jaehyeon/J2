...
namespace DigitalArena
{
    public sealed partial class DigitalArenaGame
    {
        ...
        void UpdateTouchInput()
        {
            ...
            if (touches.Count > 0)
            {
                suppressMouseUntilFrame = Time.frameCount + 2;
            }
            ...
            if (Screen.safeArea != touchSafeArea)
            {
                CancelDrag();
            }
        ...
        void EndTouch(Vector2 point)
        {
            ...
            if (!tap || mainMenu || help || rules.Health <= 0)
            {
                return;
            }
