...
namespace DigitalArena
{
    public sealed partial class DigitalArenaGame : MonoBehaviour
    {
        ...
        readonly Dictionary<int, Texture2D>	badges = new Dictionary<int, Texture2D>();
        Font								font;
        ...
        void Awake()
        {
            ...
            bool singlePlayerScene = gameObject.scene.name == GameSceneFlow.SinglePlayer;

            if (!singlePlayerScene)
            {
            ...
            menuCamera.transform.SetParent(transform);
            menuCamera.cullingMask = 0;
            menuCamera.clearFlags = CameraClearFlags.SolidColor;
            menuCamera.backgroundColor = Hex(0x112A3A);
        ...
        void StartSinglePlayer()
        {
            ...
            {
                GameSceneFlow.Load(GameSceneFlow.SinglePlayer);
            }
        }
        ...
        {
            EnterMultiplayerPreview();
        }

        void ResetRun()
        {
            mainMenu = false;
            multiplayerNotice = false;

            if (world == null)
            {
                world = gameObject.AddComponent<ArenaWorld3D>();
                world.Initialize();
            }
            ...
            if (playableCharacter == null)
            {
                ...
                if (prefab != null)
                {
                    playableCharacter = Instantiate(prefab, world.transform).GetComponent<PlayableCharacterMotor>();
                }
                else
                {
                    Debug.LogError("ShinTaeyil playable character prefab is missing.");
                }
            }

            if (playableCharacter != null)
            {
                playableCharacter.ResetPosition(new Vector3(0, .25f, -7.9f));
            }
            ...
            {
                foreach (var t in enemyPortraits)
                {
                    Destroy(t);
                }
            }

            partners = world.CreatePortraits(rules.Catalog);
            enemyPortraits = world.CreatePortraits(rules.Catalog, true);
            selectedUnit = null;
            battle = null;
            resultTime = 0;
            help = false;
        ...
        void BeginBattle()
        {
            ...
            rules.StartBattle();
            shopOpen = false;
            battle = new ArenaBattle(rules);
            world.Rebuild(rules, battle);
        }

        void Update()
        {
            SendTestEnterGameFromLobby();
            UpdateTouchInput();
            UpdateMouseInput();
            ...
            if (playableCharacter != null)
            {
                playableCharacter.Tick(Time.deltaTime, !mainMenu && !help && rules != null && rules.Health > 0);
            }

            if (mainMenu || help || rules.Health <= 0)
            {
                return;
            }
            ...
            else if (battle != null && !battle.Finished)
            {
                ...
                while (dt > 0 && !battle.Finished)
                {
                    float step = Mathf.Min(dt, 0.025f);
                ...
                if (battle.Finished)
                {
                    rules.ResolveBattle(battle.Won, battle.Survivors);
                    resultTime = 4;
                }
            }
            else if (battle != null)
        ...
        void OnDestroy()
        {
            ...
            foreach (var t in badges.Values)
            {
                Destroy(t);
            }
        ...
        void LateUpdate()
        {
            ...
            if (saleHover && dragId >= 0)
            {
                SaleOverlay();
            }

            if (battle != null && battle.Finished && !battle.Won && rules.Health > 0)
            {
                Result();
            }
        ...
        void Header()
        {
            ...
            Fill(new Rect(492, 77, 456, 3), Edge);
            Fill(new Rect(492, 77, 456 * Mathf.Clamp01(ratio), 3), Mint);
        }
        ...
        void PlayerList()
        {
            ...
            Fill(new Rect(1230, 280, 172, 4), Edge);
            Fill(new Rect(1230, 280, 172 * rules.Health / 100f, 4), Mint);
        ...
        void BottomControls()
        {
            ui.Button("economy", new Rect(24, 805, 250, 63), "골드 " + rules.Gold + "   ·   Lv. " + rules.Level, Hex(0x304C43), () =>
            {
            ...
            if (economyOpen)
            {
                ...
                Fill(new Rect(40, 545, 218, 5), Edge);
                Fill(new Rect(40, 545, 218 * (rules.Level == ArenaRules.MaxLevel ? 1 : rules.Xp / (float)rules.RequiredXp), 5), Mint);
            ...
            ui.Button("toggleShop", new Rect(1250, 805, 166, 63), shopOpen ? "기물 선택 닫기" : "기물 선택 열기", Panel, () =>
            {
                CancelDrag();
                shopOpen = !shopOpen;
            });
            ui.Button("guide", new Rect(1250, 746, 166, 45), "플레이 가이드", Panel, () =>
            {
                CancelDrag();
                help = true;
            });
        ...
        void UnitLabels()
        {
            ...
            {
                if (!unit.Root.gameObject.activeSelf || unit.Id == dragId && dragId >= 0)
                {
                    continue;
                }
                ...
                {
                    Fill(new Rect(p.x - 26, p.y, 52, 3), Panel);
                    Fill(new Rect(p.x - 26, p.y, 52 * unit.Fighter.Sp / unit.Fighter.Data.SP, 3), Hex(0x66BFF0));
                }
                ...
                if (unit.Fighter != null && unit.Fighter.Flash > 0 && unit.Fighter.Target != null)
                {
                    ...
                    for (int i = 1; i <= 8; i++)
                    {
                        Vector2 pos = Vector2.Lerp(p, target, i / 8f);

                        Fill(new Rect(pos.x, pos.y, 4, 4), unit.Enemy ? Pink : Gold);
                    }
        ...
        Texture2D Badge(DigimonData d)
        {
            ...
            if (!badges.TryGetValue(key, out var texture))
            {
                texture = DigimonBadge.Create(d.type, d.element);
                badges.Add(key, texture);
            }
        ...
        void UnitDetails()
        {
            ...
            if (selectedUnit.Root == null || !world.Units.Contains(selectedUnit))
            {
                selectedUnit = null;
            ...
            if (!u.Enemy)
            {
                ...
                for (int i = 0; i < stars; i++)
                {
                    if (rankStar != null)
                    {
                        ui.Picture(new Rect(38 + i * 28, 276, 24, 24), rankStar, ScaleMode.ScaleToFit);
                    }
                    else
                    {
                        Label(38 + i * 28, 276, 24, 24, "★", 22, Gold, true);
                    }
                }
            ...
            Fill(new Rect(38, 435, 222, 22), Edge);
            Fill(new Rect(38, 435, 222 * Mathf.Clamp01(hp / d.HP), 22), Hex(0x2A825C));
            Label(38, 435, 222, 22, "HP  " + hp.ToString("0") + " / " + d.HP.ToString("0"), 13, Ink, false, TextAnchor.MiddleCenter);
            Fill(new Rect(38, 463, 222, 22), Edge);
            Fill(new Rect(38, 463, 222 * sp / d.SP, 22), Hex(0x2D7CAC));
        ...
        void Shop()
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                Label(panel.x + 245 + i * 132, panel.y - 29, 132, 24, ArenaRules.CostNames[i] + " " + rules.Balance.levels[rules.Level - 1].weights[i] + "%", 13, Rarity(i));
            }
            ...
            for (int i = 0; i < 5; i++)
            {
                int tier = rules.Offers[i];

                Rect card = new Rect(panel.x + 16 + i * 179, panel.y + 46, 171, 114);

                if (tier < 0 || rules.Bought[i])
                {
                    PanelBox(card);
                ...
                {
                    if (rules.Offers[slot] == tier && rules.Buy(slot))
                    {
                        CancelDrag();
                        world.Rebuild(rules, null);
                    }
        ...
        {
            return new Rect(470, 18, 500, 74).Contains(point) || new Rect(1216, 166, 200, 245).Contains(point) || new Rect(24, economyOpen ? 463 : 805, 250, economyOpen ? 405 : 63).Contains(point) || selectedUnit != null && new Rect(24, 268, 250, 452).Contains(point) || new Rect(1250, 746, 166, 122).Contains(point) || shopProgress > 0 && new Rect(ShopRect.x, ShopRect.y - 32, ShopRect.width, ShopRect.height + 32).Contains(point);
        }
        ...
        bool HandlePointer(EventType type, int button, int clicks, Vector2 mouse)
        {
            ...
            {
                if (Vector2.Distance(mouseDown, mouse) > (touchFinger >= 0 ? TouchDragThreshold : 6))
                {
                    dragId = pressedId;
                }
        ...
        void SaleOverlay()
        {
            var piece = rules.Pieces.Find(p => p.Id == dragId);
        ...
        void Help()
        {
            Fill(new Rect(0, 0, 1440, 900), new Color(0, 0, 0, 0.8f));
            PanelBox(new Rect(300, 160, 840, 580));
            ...
            {
                Label(340, 247 + i * 37, 760, 28, lines[i], 15, Ink);
            }
        ...
        void GameOver()
        {
            Fill(new Rect(0, 0, 1440, 900), new Color(0, 0, 0, 0.85f));
            PanelBox(new Rect(410, 240, 620, 420));
