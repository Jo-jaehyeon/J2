...
REFERENCES = Path(r'C:/Users/User/Desktop/J2_AssetImage/ready')
RECIPES = {
...
class Builder:
    ...
    def eye(self, name, pos, size, iris, bone, outward=0, pupil=True):
        # Character-specific apertures; the eyeball alone does not define the expression.
        if hasattr(self,'detailed_eye'):
            return self.detailed_eye(name,pos,size,iris,bone,outward,pupil)
    ...
    def finalize(self, folder):
        ...
        budget=getattr(self,'triangle_budget',None)
        if budget and len(body.data.loop_triangles)>budget:raise RuntimeError('Triangle budget exceeded: '+self.name)
...
def baby(b, tsumemon=False):
    ...
    if tsumemon:
        ...
        if hasattr(b,'koromon_mouth'): b.koromon_mouth()
        else: b.grin('Smile',(0,-.491,.36),.38,.1,'Body',6)
        for s in [-1,1]:
...
def dinosaur(b, mode):
    ...
    for s in [-1,1]:
        if not advanced: b.eye('DinosaurEye',(s*.33,-.24,headz+.12),.145,'Emerald','Head',s*.7)
...
def chrysalimon(b):
    ...
    for s in [-1,1]:
        ...
        for i in range(3):
            ...
            endpoint=(s*(1.03+(i%2)*.15),-.82-i*.11,1.85-i*.53)
            b.tube('LongBlackCable',[origin,(s*.55,.70+i*.12,1.57-i*.19),(s*1.03,.70+i*.09,1.68-i*.23),(s*1.30,.18,1.68-i*.29),endpoint],[.032,.030,.028,.026,.032],'Black',name,8,sub=4)
            tip=Vector(endpoint)+Vector((s*-.18,-.45,.10))
