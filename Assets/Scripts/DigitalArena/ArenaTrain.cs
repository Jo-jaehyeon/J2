...
namespace DigitalArena
{
    public sealed class ArenaTrain
    {
        ...
        public bool Blocks(float ax, float ay, float bx, float by)
        {
            ...
            return Slab(ax, bx - ax, Center - 1.65f, Center + 1.65f, ref lo, ref hi) && Slab(ay, by - ay, 3.48f, 4.52f, ref lo, ref hi);
        }
        ...
        public void Waypoint(float ax, float ay, float bx, float by, out float x, out float y)
        {
            ...
            )
            {
                if (cx >= -0.4f && cx <= 7.4f)
                {
                    nodes.Add(new[] { cx, 3.3f });
                    nodes.Add(new[] { cx, 4.7f });
                }
            }
            ...
            for (int iteration = 0; iteration < n; iteration++)
            {
                ...
                if (current < 0 || float.IsPositiveInfinity(distance[current]))
                {
                    break;
                }
                ...
                {
                    if (visited[next] || Blocks(nodes[current][0], nodes[current][1], nodes[next][0], nodes[next][1]))
                    {
                        continue;
                    }
