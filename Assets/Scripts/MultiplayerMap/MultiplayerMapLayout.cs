...
namespace J2.MultiplayerMap
{
    ...
    public sealed class MultiplayerMapLayout : MonoBehaviour
    {
        ...
        public bool TryGetView(int arenaId, out Transform anchor, out Transform target)
        {
            ...
            if (arenaId < 0 || arenaId >= Capacity || CameraAnchors == null || CameraTargets == null || arenaId >= CameraAnchors.Length || arenaId >= CameraTargets.Length)
            {
                return false;
            }

            anchor = CameraAnchors[arenaId];
            target = CameraTargets[arenaId];
