...
namespace J2.MultiplayerMap
{
    public sealed class MultiplayerMapPreview : MonoBehaviour
    {
        ...
        public void Initialize(GameObject mapPrefab, GameObject playerPrefab)
        {
            var map = Instantiate(mapPrefab, transform);
            Layout = map.GetComponent<MultiplayerMapLayout>();
            if (Layout == null || !Layout.TryGetView(StartArena, out var anchor, out var target))
                throw new InvalidOperationException("Multiplayer map camera anchors are missing.");
            arena = Layout.Arenas[StartArena];
            Player = Instantiate(playerPrefab, arena).transform;
            Player.name = "Preview Player";
            var singlePlayerMotor = Player.GetComponent<PlayableCharacterMotor>();
            if (singlePlayerMotor != null) singlePlayerMotor.enabled = false;
            motion = Player.GetComponent<Animation>();
            Player.localPosition = destination = new Vector3(0, .25f, -7.9f);
            Player.localRotation = Quaternion.identity;
            foreach (var node in GetComponentsInChildren<Transform>(true)) node.gameObject.layer = PreviewLayer;
            foreach (var cam in FindObjectsByType<Camera>(FindObjectsSortMode.None))
                if (cam.enabled) { suspendedCameras.Add(cam); cam.enabled = false; }
            ...
            ViewCamera.nearClipPlane = .1f; ViewCamera.farClipPlane = 500;
            ...
            sun.transform.SetParent(transform, false); sun.type = LightType.Directional;
            sun.transform.rotation = Quaternion.Euler(50,-35,0); sun.intensity = 1.1f;
        ...
        public bool MoveToWorldPoint(Vector3 world)
        {
            ...
            var local = arena.InverseTransformPoint(world); local.y = .25f;
            if (!Inside(local)) return false;
            destination = local; moving = (destination - Player.localPosition).sqrMagnitude > .0001f;
            return true;
        }
        ...
        public void Tick(float dt, Func<Vector2,bool> blocked)
        {
            ...
            {
                touchStart = touch.position.ReadValue(); finger = blocked(touchStart) ? -1 : touch.touchId.ReadValue();
            }
            ...
            if (!touching && finger < 0 && mouse != null && mouse.rightButton.wasPressedThisFrame) PointAt(mouse.position.ReadValue(),blocked);
            Advance(dt);
        }
        public void Advance(float dt)
        {
            ...
            if (moving)
            {
                ...
                if ((destination - Player.localPosition).sqrMagnitude < .0001f) moving = false;
            }
            Pose(walking);
        }
        void Pose(bool walking)
        {
            ...
            if (motion != null && motion.GetClip(clip) != null && !motion.IsPlaying(clip)) motion.CrossFade(clip,.12f);
        }
        public void CancelMovement() { if(Player != null) destination=Player.localPosition; moving=false;finger=-1;Pose(false); }
        public void Shutdown()
        {
            if (stopped) return;
            stopped=true; CancelMovement();
        ...
        void OnApplicationFocus(bool focused) { if(!focused) CancelMovement(); }
        void OnApplicationPause(bool paused) { if(paused) CancelMovement(); }
