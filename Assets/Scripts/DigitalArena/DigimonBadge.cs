...
namespace DigitalArena
{
    ...
    {
        public static readonly Color[] Colors={new Color32(234,64,55,255),new Color32(40,114,237,255),new Color32(155,225,59,255),new Color32(250,219,53,255),new Color32(119,213,248,255),new Color32(147,92,50,255),Color.white,Color.black,Color.gray};
        static readonly string[] Names={"Vaccine","Virus","Data","Unknown","Free","NoData"};
        ...
        {
            var pixels=source.GetPixels();int left=source.width,right=-1,bottom=source.height,top=-1;
            for(int y=0;y<source.height;y++)for(int x=0;x<source.width;x++)
            {
                Color p=pixels[y*source.width+x];
                if((Mathf.Max(p.r,p.g)-p.b)*p.a<.1f)continue;
                left=Mathf.Min(left,x);right=Mathf.Max(right,x);bottom=Mathf.Min(bottom,y);top=Mathf.Max(top,y);
            }
            if(right<left) throw new System.InvalidOperationException("No visible type symbol: "+source.name);
        ...
        public static Texture2D Create(DigimonType type,DigimonElement element)
        {
            ...
            if(source==null) throw new System.InvalidOperationException("Missing type texture: "+Names[(int)type]);
            ...
            try
            {
                ...
                var result=new Texture2D(64,64,TextureFormat.RGBA32,false){name=Names[(int)type]+"_"+element,filterMode=FilterMode.Bilinear};
                result.ReadPixels(new Rect(0,0,64,64),0,0);result.Apply();return result;
            }
            finally { RenderTexture.active=previous;RenderTexture.ReleaseTemporary(target);Object.Destroy(material); }
        }
    }
}
