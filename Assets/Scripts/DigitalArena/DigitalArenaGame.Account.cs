...
namespace DigitalArena
{
    public sealed partial class DigitalArenaGame
    {
        ...
        [SerializeField]
        bool	externalLoginEnabled = false;
        ...
        {
            account = FindAnyObjectByType<ArenaAccountClient>();

            if (account == null)
            {
                var session = new GameObject("Account Session");

                account = session.AddComponent<ArenaAccountClient>();
        ...
        void AccountMenu()
        {
            ...
            if (account.Profile == null)
            {
                ...
                if (account.Busy)
                {
                    ui.Button("cancelLogin", new Rect(620, 711, 200, 44), "로그인 취소", Panel, account.CancelLogin);
                }
            }
