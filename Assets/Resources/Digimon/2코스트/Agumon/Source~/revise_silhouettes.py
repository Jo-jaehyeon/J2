...
def loft(b,name,sections,color,bone,sides=16):
    ...
    for y,w,low,high in sections:
        for i in range(sides):
            ...
            if name=='SculptedAgumonHead':
                c=math.copysign(abs(c)**.70,c)*(.91+.09*t)
                t=math.copysign(abs(t)**.67,t)
            vs.append((w*c,y,(low+high)/2+(high-low)/2*t))
...
class SkullBuilder(RevisedBuilder):
    agumon_eye_surface=True
    def tube(self,name,*args,**kwargs):
        if self.name=='Agumon' and name in ['UpperFang','LowerFang']:return None
        return super().tube(name,*args,**kwargs)
    def oval(self,name,*args,**kwargs):
        if name in ['Cranium','LongMuzzle','SkullHelmet','HelmetSnout']:return None
        if self.name=='Agumon' and name=='Belly':return None
        if self.name=='Agumon' and name=='Nostril':
            side=-1 if args[0][0]<0 else 1
            return Builder.oval(self,name,(side*.12,-.813,1.46),(.012,.005,.009),'HornBrown','Head',8,5)
        if self.name=='Agumon' and name=='MouthCavity':
            return loft(self,'NarrowMouthGap',[(0,.17,1.25,1.28),(-.28,.28,1.245,1.27),(-.52,.21,1.245,1.27),(-.73,.15,1.265,1.29)],'Mouth','Head')
...
def skull_dinosaur(b):
    ...
    if b.name=='Agumon':
        ...
        loft(b,'SculptedAgumonHead',[(.30,.13,1.36,1.87),(.22,.28,1.30,1.98),(.06,.38,1.27,2.005),
            ...
            (-.62,.24,1.27,1.62),(-.76,.195,1.275,1.55),(-.81,.18,1.28,1.525)],'SkinGold','Head',20)
        ...
        for s in [-1,1]:
            ...
            for y,x in [(-.58,.18),(-.38,.25)]:
                b.tube('InsetSmallFang',[(s*x,y,1.275),(s*x,y,1.24)],[.013,.001],'Ivory','Head',5)
    else:
