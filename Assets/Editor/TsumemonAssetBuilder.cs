...
public static class TsumemonAssetBuilder
{
    ...
    {
        Directory.CreateDirectory(Folder);meshId=0;
        ...
        float[] angles={0,-65,65,-130,130};
        for(int i=0;i<5;i++)
        {
            float a=angles[i]*Mathf.Deg2Rad;var axis=new Vector3(Mathf.Sin(a),0,Mathf.Cos(a));
            var leg=Node(rig,"Leg"+i,axis*.37f+Vector3.up*.5f);leg.localRotation=Quaternion.Euler(0,angles[i],0);
        ...
        foreach(string name in new[]{"Idle","Walk","Attack","Special","Hit","Death"})
        {
            var clip=new AnimationClip {name=name,legacy=true,frameRate=30,wrapMode=name=="Idle"||name=="Walk"?WrapMode.Loop:WrapMode.ClampForever};
            ...
            string clipPath=Folder+"/"+name+".anim";Save(clip,clipPath);animation.AddClip(AssetDatabase.LoadAssetAtPath<AnimationClip>(clipPath),name);
        }
        animation.clip=animation.GetClip("Idle");root.AddComponent<ArenaUnitAnimation>();
        ...
        if(row==null) throw new Exception("Tsumemon entry missing");
        row.prefabPath="Digimon/Tsumemon/Tsumemon";
        File.WriteAllText(catalogPath,JsonUtility.ToJson(catalog,true));AssetDatabase.ImportAsset(catalogPath);
    ...
    static void Save(UnityEngine.Object obj,string path)
    {
        ...
        else { EditorUtility.CopySerialized(obj,existing);UnityEngine.Object.DestroyImmediate(obj);EditorUtility.SetDirty(existing); }
    }
    static Material Material(string name,Color color)
    {
        var mat=new Material(Shader.Find("DigitalArena/Solid")){name=name,color=color};string path=Folder+"/"+name+".mat";Save(mat,path);return AssetDatabase.LoadAssetAtPath<Material>(path);
    }
    ...
    {
        var flat=new Vector3[indices.Count];for(int i=0;i<flat.Length;i++) flat[i]=vertices[indices[i]];
        var mesh=new Mesh {name=name,vertices=flat,triangles=Enumerable.Range(0,flat.Length).ToArray()};mesh.RecalculateNormals();mesh.RecalculateBounds();
        string path=Folder+"/Mesh"+(meshId++)+"_"+name+".asset";Save(mesh,path);
        var t=Node(parent,name,Vector3.zero);t.gameObject.AddComponent<MeshFilter>().sharedMesh=AssetDatabase.LoadAssetAtPath<Mesh>(path);t.gameObject.AddComponent<MeshRenderer>().sharedMaterial=material;
    }
    ...
    static void Tube(Transform parent,string name,Vector3[] points,float[] radii,Material material,float thickness)
    {
        ...
        for(int p=0;p<points.Length;p++)
        {
            ...
            for(int i=0;i<sides;i++) { float angle=i*Mathf.PI*2/sides;vertices.Add(points[p]+radii[p]*(right*Mathf.Cos(angle)+up*Mathf.Sin(angle)*thickness)); }
        }
        for(int p=0;p<points.Length-1;p++)for(int i=0;i<sides;i++)
        { int a=p*sides+i,b=p*sides+(i+1)%sides;triangles.AddRange(new[]{a,b,a+sides,b,b+sides,a+sides}); }
        for(int i=1;i<sides-1;i++) { triangles.AddRange(new[]{0,i+1,i});int a=(points.Length-1)*sides;triangles.AddRange(new[]{a,a+i,a+i+1}); }
        MeshObject(parent,name,vertices,triangles,material);
    }
    ...
    static void Render(GameObject root)
    {
        ...
        var cameraObject=new GameObject("Asset preview camera");var cam=cameraObject.AddComponent<Camera>();cam.enabled=false;cam.cullingMask=1<<29;
        cam.clearFlags=CameraClearFlags.SolidColor;cam.backgroundColor=new Color(.13f,.18f,.23f);cam.orthographic=true;cam.orthographicSize=1.55f;
        var target=new RenderTexture(768,768,24);cam.targetTexture=target;var prior=RenderTexture.active;
        foreach(var view in new[]{"Front","Side","Back","ThreeQuarter"})
        {
            ...
            cam.transform.LookAt(new Vector3(0,1.15f,0));cam.Render();RenderTexture.active=target;
            var texture=new Texture2D(768,768,TextureFormat.RGB24,false);texture.ReadPixels(new Rect(0,0,768,768),0,0);texture.Apply();File.WriteAllBytes("Logs/Tsumemon/"+view+".png",texture.EncodeToPNG());UnityEngine.Object.DestroyImmediate(texture);
        }
        RenderTexture.active=prior;cam.targetTexture=null;target.Release();UnityEngine.Object.DestroyImmediate(target);UnityEngine.Object.DestroyImmediate(cameraObject);
    }
}
