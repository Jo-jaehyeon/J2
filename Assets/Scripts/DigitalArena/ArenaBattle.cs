...
namespace DigitalArena
{
    public sealed class ArenaBattle
    {
        ...
        public static int WorldRow(int cell)
        {
            int row = cell / 8;
        ...
        public ArenaBattle(ArenaRules rules)
        {
            ...
            {
                Add(rules, piece.Tier, false, (3 - piece.Cell / 8) * 8 + piece.Cell % 8, piece.Stars);
            }

            for (int i = 0; i < EnemyCount(rules.Stage, rules.StageRound); i++)
            {
                Add(rules, rules.Catalog.EnemyAt(rules.Stage, i), true, 32 + i);
            }
        }
        ...
        {
            if (Occupied(to, unit) || !Clear(from, to))
            {
                return false;
            }
            ...
            if (from % 8 != to % 8 && from / 8 != to / 8)
            {
                ...
                if (Occupied(sideA, unit) || Occupied(sideB, unit) || !Clear(from, sideA) || !Clear(from, sideB) || !Clear(sideA, to) || !Clear(sideB, to))
                {
                    return false;
                }
            }
        ...
        {
            var previous = Enumerable.Repeat(-1, 64).ToArray();
            ...
            while (queue.Count > 0)
            {
                ...
                for (int dy = -1; dy <= 1; dy++)
                {
                    for (int dx = -1; dx <= 1; dx++)
                    {
                        ...
                        if (previous[next] >= 0 || !CanStep(cell, next, unit))
                        {
                            continue;
                        }
        ...
        public void Tick(float dt)
        {
            ...
            foreach (var f in Fighters)
            {
                ...
                if (target != null)
                {
                    ...
                    f.Cooldown = f.Data.modelTier >= 5 ? .7f : .95f;
                    f.Flash = .22f;
                }
                else if (f.Data.SPD > 0)
                {
                    if (enemies.Any(t => InRange(f.Cell, t, f.Data.range)))
                    {
                        continue;
                    }
