...
def detailed_eye(self,name,pos,size,iris,bone,outward=0,pupil=True):
    ...
    species=self.name
    p+=n*({'Agumon':.10,'Greymon':.055,'MetalGreymon':.055,'Koromon':.025}.get(species,0))
    if species=='Tsumemon':
    ...
    patch('Aperture',outline,'Black',.008)
    ...
    if species=='Keramon':
        ...
        rx=.52 if species not in ['Diaboromon','Chrysalimon'] else .36
        rz=.63 if species in ['Agumon','Koromon'] else .43
...
def koromon_mouth(self):
    ...
    pts=[surf(-.17,.225,.059),surf(0,.20,.062),surf(.17,.225,.059)]
    self.tube('TongueInsideMouth',pts,[.045,.055,.045],'PinkShadow','Body',8,flat=.40,sub=3)
